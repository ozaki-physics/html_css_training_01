...
### 文字やテキストについて
  __伝えたいイメージに合わせて適切なフォントを選ぶ__<br>
  ウロコとは、「はね」「はらい」などの装飾がある筆で書いたような文字
  和文フォントには大きく2種類
  - 明朝体<br>
    ウロコあり
  - ゴシック体<br>
  ...
  行間が詰まっていると、圧迫感。行間が空いていると、間延びした印象。<br>
  本文であれば行間は1文字サイズの半分程度が良い
### 文字コード
  __UTF-8が主流__<br>
  世界中の文字を一括して登録を目標としたのが、Unicode<br>
  UTF-8はUnicodeの一種
### 色
  表現方法は3種類
  - RGB<br>
    赤(red)、緑(green)、青(blue)を0から255の256段階で表す。RGBを16進数で置き換えて6桁の文字で色を表すカラーコードを使用する。
  - RGBA<br>
    RGBに透明度(Alpha)を加えたもの。半透明の背景などで使用する。
  - HSLA<br>
    CSS3では、色相、彩度、明度、透明度で指定することもできる。
### Webデザインの単位
  サイズ指定で使う単位の種類 ブラウザの標準フォントサイズは16pxらしい?
  - px<br>
    画面の1ピクセルの長さを1とする。ブラウザの横幅に関係なく同じサイズで表示される。
  - %<br>
    相対的なサイズ指定。柔軟にサイズを変えたいレスポンシブデザインで使用されることが多い。親要素の文字サイズを100%とする。
  - em<br>
    相対的なサイズ指定。親要素の文字サイズを1emとする。
  - rem<br>
    相対的なサイズ指定。html要素の文字サイズを1remとする。
  - vw<br>
    viewport width<br>
    Webブラウザ全体の横幅を100vwとする。%との違いは、%は親要素に対する割合のため、親要素の横幅の影響を受けてしまう
### ボックスモデル
  ボックスとは、Webページを構成する要素の四角形の領域のこと<br>
  コンテンツ自体の高さをheight(ハイト)、幅をwidth(ウィドゥス)<br>
  内側の余白をpaddig(パディング)、枠線をborder(ボーダー)、外側の余白をmargin(マージン)
### Webデザインのトレンド
  フラットデザインとは、装飾の無いただの平面<br>
  リッチデザインとは、影が付いていたりグラデーション、エンボス加工した現実的な状態<br>
  スマホ用にシングルカラム<br>
  遅い回線でも大丈夫なように軽いWebサイト
### カラーコードのオススメサイト
  原色大辞典：https://www.colordic.org/<br>
